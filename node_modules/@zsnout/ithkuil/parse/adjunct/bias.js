import { BIAS_ITHKUIL_TO_ADJUNCT_MAP, } from "../../generate/adjunct/bias.js";
 ...
 */
export function buildBiasAdjunct(word) {
 ...
 */
export function dcBiasAdjunct(word) {
    const match = biasAdjunct.exec(word);
    if (!match) {
        return;
    }
    if (Object.hasOwn(BIAS_ITHKUIL_TO_ADJUNCT_MAP, match[0])) {
        return new DCWord("adjBias", new DCStress("zerosyllabic", null), new DCLeaf(match[0], "Cb", "bias", BIAS_ITHKUIL_TO_ADJUNCT_MAP[match[0]]));
    }
    throw new Error("Unknown Bias: " + match[0] + ".");
}
