...
import { dcMoodOrCaseScope, parseMoodOrCaseScope, } from "../formative/mood-or-case-scope.js";
import { dcAspect, dcNonAspectualVn, parseAspect, parseNonAspectualVn, } from "../formative/vn.js";
 ...
 *   etc.).
 */
export function buildModularAdjunct(word, stress) {
    const match = modularAdjunct.exec(word);
    if (!match) {
        return;
    }
    const type = match[1] == "w" ? "PARENT"
        : match[1] == "y" ? "CONCAT"
            : undefined;
    if (!(match[2] || match[4])) {
        return {
 ...
 *   etc.).
 */
export function dcModularAdjunct(word, stress) {
    const match = modularAdjunct.exec(word);
    if (!match) {
        return;
    }
    const type = match[1] == "w" ? [new DCLeaf("w", "C*", "modularType", "PARENT")]
        : match[1] == "y" ? [new DCLeaf("y", "C*", "modularType", "CONCAT")]
            : [];
    if (!(match[2] || match[4])) {
        return new DCWord("adjModular", new DCStress("monosyllabic", "VN"), ...type, new DCLeaf(match[6], "Vn", "aspect", parseAspect(VowelForm.parseOrThrow(match[6]))));
    }
    ...
    if (stress == "ultimate") {
        ...
        return new DCWord("adjModular", new DCStress(stress, "VH"), ...type, vn1, cn, ...vn2, new DCLeaf(match[6], "Vh", "modularScope", scope));
    }
    return new DCWord("adjModular", new DCStress(stress, "VN"), ...type, vn1, cn, ...vn2, dcNonAspectualVn(match[6]));
}
