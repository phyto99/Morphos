...
/* @__PURE__ */ seq(SLOT_IV_DATA, 
/* @__PURE__ */ text("."), SLOT_IV_DATA, 
...
/* @__PURE__ */ anyTextArray(ALL_CASE_SCOPES))))
    .matchEntireText()
    .compile();
function substituteLetters(word) {
    return word.replace(LETTER_SUBSTITUTION_REGEX, (letter) => LETTER_SUBSTITUTIONS[letter]);
}
// Consonants
...
const Referent = /* @__PURE__ */ seq(ReferentTarget, 
/* @__PURE__ */ anyText(".BEN", ".NEU", ".DET", ":BEN", ":NEU", ":DET", ".ben", ".neu", ".det", ":ben", ":neu", ":det").optional());
const ReferentList = /* @__PURE__ */ any(Referent, 
/* @__PURE__ */ seq(
/* @__PURE__ */ text("["), Referent, 
/* @__PURE__ */ seq(/* @__PURE__ */ text("+"), Referent).zeroOrMore(), 
/* @__PURE__ */ text("]")));
const referentListRegex = 
/* @__PURE__ */ ReferentList.matchEntireText().compile();
const referentRegex = /* @__PURE__ */ seq(text("("), Referent, seq(text("+"), Referent).zeroOrMore(), anyText("+M", "+G", "+N").optional(), text("-"), anyTextArray(ALL_REFERENTIAL_AFFIX_CASES), text(")"))
    .matchEntireText()
    .compile();
// Affixes
...
const AnyCaslot = /* @__PURE__ */ any(Affiliation, Configuration, Extension, Perspective, Essence);
...
const caseRelatedRegex = /* @__PURE__ */ seq(anyText("(acc:", "(ia:", "(case:", "("), Case, text(")"), charIn("123₁₂₃").optional())
    .matchEntireText()
    .compile();
// General
const segmentSplitterRegex = /* @__PURE__ */ seq(text("-"), seq(anyTextArray(ALL_REFERENTIAL_AFFIX_CASES), text(")")).not()).compile("g");
function parseCaseRelatedGloss(gloss) {
...
export function parseCaGloss(ca) {
...
function parseReferentGloss(referent) {
    let effect = "NEU";
...
function parseReferentListGloss(list) {
    if (list.startsWith("[")) {
        list = list.slice(1, -1);
    }
...
function parseReferentialAffixGloss(affix) {
    ...
    const referents = affix.split("+").map(parseReferentGloss);
    return {
        referents,
        perspective,
        case: case_,
...
function tailSegmentToAffix(segment) {
    ...
    return segments.map((segment) => {
        ...
        if (segment.startsWith("(")) {
            ...
            if (affixRegex.test(segment)) {
                return {
                    type: "affix",
                    affix: parseAffixGloss(segment + end),
                };
            }
            segment = "(" + segment + ")" + end;
        ...
        if (affixRegex.test(segment)) {
            return {
                type: "affix",
                affix: parseAffixGloss(segment),
            };
        }
        throw new Error("Invalid tail segment: '" + segment + "'.");
    });
}
function unglossTailSegments(segments, type) {
    const affixes = parseTailSegments(segments);
    let case_;
    let illocutionValidation;
 ...
 *   Ca.
 */
export function unglossFormative(gloss) {
    ...
    if (Array.isArray(root)) {
        if ((stem ?? 1) != 1) {
            throw new Error("Cannot specify non-stem 1 on a specialized personal-reference root formative.");
        }
        if (function_ == "DYN") {
            throw new Error("Cannot specify non-STA function on a specialized personal-reference root formative.");
        }
        if (caShortcut) {
            if ((caShortcut.affiliation || "CSL") != "CSL" ||
                ...
                throw new Error("Invalid CA shortcut on a specialized personal-reference root formative. Only [default] and PRX are allowed.");
            }
        }
    }
    else if (typeof root == "object") {
        if (stem != null) {
            throw new Error("Cannot specify stem on a specialized affixual-root formative.");
        }
        if ((specification || "BSC") != "BSC") {
            throw new Error("Cannot specify non-BSC specification on a specialized affixual-root formative.");
        }
        if (caShortcut) {
            throw new Error("Cannot specify a CA shortcut on a specialized affixual-root formative.");
        }
    }
    if (affixShortcut && caShortcut) {
        ...
        if (emptyCaSplitIndex != -1) {
            ...
            if (caSplitIndex != -1) {
                ...
                if (type == "UNF/K") {
                    ...
                    if (!vn && !caseScope) {
                        const caseScopeSplitIndex = affixes.findIndex((x) => x.type == "viCaseScope");
    ...
    if (slotVAffixes &&
        ...
        slotVAffixes.some((affix) => affix.referents && has(TYPE_3_REFERENTIAL_AFFIX_CASES, affix.case))) {
        throw new Error("Cannot mix a referential affix with " +
            slotVAffixes.find((affix) => affix.referents && has(TYPE_3_REFERENTIAL_AFFIX_CASES, affix.case))?.case +
    ...
    if (slotVIIAffixes &&
        ...
        slotVIIAffixes.some((affix) => affix.referents && has(TYPE_3_REFERENTIAL_AFFIX_CASES, affix.case))) {
        throw new Error("Cannot mix a referential affix with " +
            slotVIIAffixes.find((affix) => affix.referents && has(TYPE_3_REFERENTIAL_AFFIX_CASES, affix.case))?.case +
            " case with other affixes in slot VII.");
    }
    return {
        type,
        shortcut: caShortcut ? "IV/VI"
...
/* @__PURE__ */ PlusPerspective.optional()))
    .matchEntireText()
    .compile();
function parseReferentialReferentGloss(gloss) {
    let perspective;
    if (gloss.startsWith("[")) {
        gloss = gloss.slice(1);
    }
    ...
    if (gloss.endsWith("+M") ||
        ...
        gloss.endsWith("+N") ||
        gloss.endsWith("+A")) {
        perspective = gloss.slice(-1);
        gloss = gloss.slice(0, -2);
    }
    if (gloss.endsWith("]")) {
        gloss = gloss.slice(0, -1);
    }
    const referents = gloss.split("+").map(parseReferentGloss);
    return [referents, perspective];
}
/**
 ...
 */
export function unglossReferential(gloss) {
    gloss = substituteLetters(gloss);
    let essence = "NRM";
    ...
    if (has(ALL_CASES, secondSegment)) {
        ...
        if (segments.length == 2 &&
            ...
            const [referents2, perspective2] = parseReferentialReferentGloss(segments[1]);
            return {
                ...core,
                essence,
                case: case_,
 ...
 */
export function unglossSimpleAdjunct(gloss) {
    gloss = substituteLetters(gloss);
    if (n.test(gloss)) {
    ...
    if (gloss.length == 7 &&
        ...
        const register = gloss.slice(0, 3);
        if (register == "NRR") {
 ...
 *   - `r/1-r/3-VII:DOM`
 */
export function unglossAffixualAdjunct(gloss) {
    gloss = substituteLetters(gloss);
    ...
    if (scope2 && affixes.length == 1) {
        throw new Error("Cannot specify two affixual adjunct scopes when only one affix is present.");
    }
    return {
        affixes: affixes,
        scope,
 ...
 *   modular adjunct.
 */
export function unglossModularAdjunct(gloss) {
    gloss = substituteLetters(gloss);
    let type;
    let scope;
    const segments = gloss.split(segmentSplitterRegex).filter((segment) => {
        ...
        if ((segment.includes("case") && segment.includes("mood")) ||
 ...
 */
export function unglossWord(gloss) {
    gloss = substituteLetters(gloss);
    function attemptGloss(label, ungloss) {
        try {
