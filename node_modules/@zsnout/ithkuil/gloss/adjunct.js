import { ALL_BIAS_ADJUNCTS, ALL_PARSING_ADJUNCTS, ALL_REGISTER_ADJUNCTS, ALL_SINGLE_REGISTER_ADJUNCTS, BIAS_ADJUNCT_TO_NAME_MAP, CASE_TO_NAME_MAP, REGISTER_ADJUNCT_TO_NAME_MAP, SUPPLETIVE_ADJUNCT_TYPE_TO_NAME_MAP, has, } from "../generate/index.js";
 ...
 */
export function glossAdjunct(adjunct) {
    ...
    if (typeof adjunct == "string") {
        if (has(ALL_BIAS_ADJUNCTS, adjunct)) {
            return new GlossString(adjunct, BIAS_ADJUNCT_TO_NAME_MAP[adjunct].toLowerCase());
        }
        if (has(ALL_PARSING_ADJUNCTS, adjunct)) {
            return new GlossString(adjunct == "monosyllabic" ? "mono:"
                : adjunct == "ultimate" ? "ulti:"
                    : adjunct == "antepenultimate" ? "ante:"
                        : "penu:", adjunct);
        }
        if (has(ALL_SINGLE_REGISTER_ADJUNCTS, adjunct)) {
            return new GlossString(adjunct.endsWith(":START") ?
                adjunct.slice(0, 3)
                : adjunct.slice(0, 3) + "_END", adjunct.endsWith(":START") ?
                REGISTER_ADJUNCT_TO_NAME_MAP[adjunct.slice(0, 3)].toLowerCase()
                : REGISTER_ADJUNCT_TO_NAME_MAP[adjunct.slice(0, 3)].toLowerCase() + "_end");
        }
    ...
    if ("affixes" in adjunct && adjunct.affixes) {
        ...
        const scope = GlossString.of(adjunct.scope == "V:DOM" || !adjunct.scope ?
            ""
            : "{" +
                (adjunct.scope == "FORMATIVE" ? "formative"
                    : adjunct.scope == "ADJACENT" ? "adjacent"
                        : adjunct.scope) +
                "}");
        const otherAffixes = adjunct.affixes.length > 1 ?
            adjunct.affixes
                .slice(1)
                .map((x) => glossAffix(x, false))
                .reduce((a, b) => a.plusString("-").plusGloss(b))
            : GlossString.of("");
        const scope2 = GlossString.of(adjunct.scope2 == "V:DOM" || !adjunct.scope2 ?
            ""
            : "{" +
        ...
        const concatOnly = adjunct.appliesToConcatenatedStemOnly ?
            ...
            : GlossString.of("");
        return [firstAffix, scope, otherAffixes, scope2, concatOnly].reduce((a, b) => (b.isEmpty() ? a : a.plusString("-").plusGloss(b)));
    }
    if ("vn1" in adjunct && adjunct.vn1) {
        ...
        if (adjunct.type == "PARENT") {
            ...
            segments.push(new GlossString("{concat.}", "{concatenated formative only}"));
        }
        if (adjunct.vn1) {
        ...
        if (adjunct.scope == "FORMATIVE") {
            segments.push(new GlossString("{under adj.}", "{scope over formative, but not adjacent adjuncts}"));
        }
        else if (adjunct.scope == "ADJACENT") {
            segments.push(new GlossString("{over adj.}", "{scope over formative and adjacent adjuncts}"));
        }
    ...
    if ("type" in adjunct && adjunct.type) {
        return new GlossString("[" +
            adjunct.type +
            "]" +
            (adjunct.case == "THM" || !adjunct.case ? "" : "-" + adjunct.case), SUPPLETIVE_ADJUNCT_TYPE_TO_NAME_MAP[adjunct.type].toLowerCase() +
