...
import { ALL_ILLOCUTIONS, CASE_TO_NAME_MAP, CONTEXT_TO_NAME_MAP, ILLOCUTION_TO_NAME_MAP, SPECIFICATION_TO_NAME_MAP, VALIDATION_TO_NAME_MAP, has, } from "../generate/index.js";
 ...
 */
export function glossFormative(formative) {
    // Slot I
    const slot1 = formative.type == "UNF/C" ?
    ...
    if (formative.shortcut == "VII" ||
        ...
        if (((finalSlotVIIAffix?.cs == "t" &&
            ...
            slotVIIAffixes.pop();
            additionalSlot2Information = GlossString.of(".").plusGloss(glossAffix(finalSlotVIIAffix, true));
            didSlotVIIShortcut = true;
        }
    }
    let didCaShortcut = false;
    if (formative.shortcut == "IV/VI" ||
        ...
        if (!additionalSlot2Information.isEmpty()) {
            additionalSlot2Information = GlossString.of(".").plusGloss(additionalSlot2Information);
        }
        didCaShortcut = true;
    }
    const version = formative.version == "CPT" ?
        new GlossString(".CPT", ".completive")
        : GlossString.of("");
    const stem = new GlossString("S" + (formative.stem ?? 1), "stem_" + (["zero", "one", "two", "three"][formative.stem ?? 1] || "one"));
    ...
    if (isArray(formative.root)) {
        ...
        const associatedRoot = typeof formative.root == "number" || typeof formative.root == "bigint" ?
            getIntegerRootEntry(BigInt(formative.root))
            : rootsMap.get(formative.root.replace(/_/g, ""));
        if (associatedRoot) {
            const root = associatedRoot.stems[formative.stem ?? 1] || associatedRoot.stems[0];
    ...
    const slot4 = [
        ...
        formative.specification && formative.specification != "BSC" ?
            new GlossString(formative.specification, SPECIFICATION_TO_NAME_MAP[formative.specification].toLowerCase())
            : GlossString.of(""),
        formative.context && formative.context != "EXS" ?
            new GlossString(formative.context, CONTEXT_TO_NAME_MAP[formative.context].toLowerCase())
            : GlossString.of(""),
    ].reduce((a, b) => !a.isEmpty() && !b.isEmpty() ? a.plusString(".").plusGloss(b)
        : a.isEmpty() ? b
            : a);
    // Slot V
    ...
    if (!didSlotVIIIShortcut) {
        const vn = !formative.vn || formative.vn == "MNO" ? undefined : glossVn(formative.vn);
        const cnForm = formative.type == "UNF/K" ? formative.mood : formative.caseScope;
        const cn = cnForm && cnForm != "FAC" && cnForm != "CCN" ?
    ...
    if (formative.type == "UNF/K") {
        slot9 = new GlossString(formative.illocutionValidation || "OBS", has(ALL_ILLOCUTIONS, formative.illocutionValidation) ?
            asGloss(ILLOCUTION_TO_NAME_MAP[formative.illocutionValidation])
            : asGloss(VALIDATION_TO_NAME_MAP[formative.illocutionValidation || "OBS"]));
    }
    else {
        if (formative.case && formative.case != "THM") {
            slot9 = new GlossString(formative.case, asGloss(CASE_TO_NAME_MAP[formative.case]));
        }
    }
    // Result
    const result = [
