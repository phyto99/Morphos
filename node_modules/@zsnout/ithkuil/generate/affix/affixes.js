import { ALL_ASPECTS, ALL_CASES, ALL_CASE_SCOPES, ALL_EFFECTS, ALL_ILLOCUTIONS, ALL_LEVELS, ALL_MOODS, ALL_PHASES, ALL_VALENCES, ALL_VALIDATIONS, has, } from "../index.js";
 ...
 */
export function vnToAffix(vn) {
    if (has(ALL_VALENCES, vn)) {
        return {
            cs: VAL,
            degree: (ALL_VALENCES.indexOf(vn) + 1),
            type: 1,
        };
    }
    if (has(ALL_PHASES, vn)) {
        return {
            cs: PHS,
            degree: (ALL_PHASES.indexOf(vn) + 1),
            type: 1,
        };
    }
    if (has(ALL_EFFECTS, vn)) {
        return {
            cs: EFE,
            degree: (ALL_EFFECTS.indexOf(vn) + 1),
            type: 1,
        };
    }
    if (has(ALL_LEVELS, vn)) {
        return {
            cs: LVL,
            degree: (ALL_LEVELS.indexOf(vn) + 1),
            type: 1,
        };
    }
...
export function toAffix(item) {
    ...
    if (has(ALL_ILLOCUTIONS, item)) {
        return {
            cs: IVL,
            degree: (ALL_ILLOCUTIONS.indexOf(item) + 1),
            type: 1,
        };
    }
    if (has(ALL_VALIDATIONS, item)) {
        return {
            cs: IVL,
            degree: (ALL_VALIDATIONS.indexOf(item) + 1),
            type: 2,
        };
    }
    ...
    if (has(ALL_CASE_SCOPES, item)) {
        ...
        return {
            cs: MCS,
            degree: (ALL_CASE_SCOPES.indexOf(item) + (5 % 10)),
            type: 1,
        };
    }
    if (item == "NEG/4") {
