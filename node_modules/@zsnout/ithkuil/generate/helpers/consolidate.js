...
import { ALL_CASE_SCOPES, ALL_MOODS, MCS, deepFreeze, has, vnToAffix, } from "../index.js";
 ...
 */
export function mergeAdjunctsAndFormative(adjuncts, formative) {
    ...
    for (const adjunct of adjuncts) {
        if ("vn1" in adjunct) {
            ...
            if (adjunct.cn) {
                const index = has(ALL_MOODS, adjunct.cn) ? ALL_MOODS.indexOf(adjunct.cn)
            ...
            if (adjunct.affixes.length > 1) {
                const scope = SCOPE_TO_PRECEDENCE[adjunct.scope2 || adjunct.scope || "V:DOM"];
                const affixList = affixes[scope];
