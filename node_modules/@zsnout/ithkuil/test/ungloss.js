...
import { unglossAffixualAdjunct, unglossFormative, unglossModularAdjunct, unglossReferential, unglossSimpleAdjunct, } from "../ungloss/index.js";
...
function show(inputGloss, label, ungloss) {
    try {
    ...
    if (recognized.gloss != inputGloss) {
        ...
        for (const r of recognized.replacements) {
            ...
            if (r.kind == "root") {
                ...
                write(r.actual.cs + "/" + r.actual.degree, r.alts.map((x) => x.value.slice(0, 20)).join(", "));
            }
            else {
                write(r.actual.cs + "/" + r.degree, r.alts
                    .map((x) => (x.degrees[r.degree] || x.description || x.abbreviation).slice(0, 20))
                    .join(", "));
            }
        }
    }
